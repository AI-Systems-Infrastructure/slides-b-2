...
In Module B.1 we looked at the \textit{hardware side of running AI}: how a generic computer is built, why that design struggles with AI workloads, and what accelerators bridge the gap.

    \vspace{0.5em}
The next question is \textbf{how we actually get our software onto all that hardware}.
\end{frame}
...
Each virtual machine has its own kernel and operating system, while containers share the same host kernel.
...
\begin{frame}{Layered filesystems}
    An image holds everything a program needs, which adds up to a lot of files.
    Storing each image as one solid block would waste space, since most images have a lot in common.
    Two different Python apps still sit on the same Python and the same Linux underneath.

    \begin{figure}[htpb]
        \begin{center}
            \includegraphics[width=0.3\textwidth]{danish-burger}
        \end{center}
    \end{figure}

    A fast-food kitchen does not build each burger from scratch.
    The bottom bun and patty are shared, a cheeseburger adds cheese, a deluxe burger adds lettuce, tomato, and sauce.
\end{frame}

\begin{frame}[fragile]{An image is a stack of layers}
    An image is built like those burgers, as a stack of layers, each adding a bit on top of the one below.
    \begin{minted}{text}
4. Application code   <- the special sauce, your own program
3. Python packages    <- the extra toppings
2. Python runtime     <- the patty
1. Linux base         <- the bottom bun
    \end{minted}

    \begin{itemize}
        \item A shared layer is stored only once, however many images use it
        \item Pulling a new image only downloads the layers you do not already have
    \end{itemize}

    So the second image built on the same base feels much faster to pull than the first.
\end{frame}
...
\begin{frame}{Run the A.2 client in a container}
    ...
    \begin{block}{Outline}
        \begin{enumerate}
            ...
            \item Run a container from an image with a Python runtime, like \texttt{python:3.13}, with volume mapping to pass the program
...
\begin{frame}{What you should walk away with}
    ...
    \begin{enumerate}
        \item Understand how containers work
        \item Be familiar with the Docker ecosystem
